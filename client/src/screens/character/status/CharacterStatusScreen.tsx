...
const BarRow: React.FC<{
  ...
  <View style={styles.barRow}>
    ...
    <View style={styles.barWrapper}>
      <View style={[styles.barFill, { backgroundColor: color, width: `${Math.min(pct, 100)}%` as `${number}%` }]} />
      <Text style={styles.barText}>{text}</Text>
    </View>
  </View>
);

const InfoRow: React.FC<{ label: string; value: string; valueRed?: boolean }> = ({ label, value, valueRed }) => (
...
const AttrRow: React.FC<{ label: string; value: number; primary?: boolean }> = ({ label, value, primary }) => (
...
export const CharacterStatusScreen: React.FC<StatusScreenProps> = ({
  ...
  const player = useMemo(() => ({
    ...
    points: appearance.freePoints ?? appearance.points ?? 0,
    combat: appearance.combat ?? { attack: 0, def: 0, acc: 0, dodge: 0, hp: 0, crit: '0%' },
  }), [appearance]);

  const STAT_LABELS = ['Cường Lực', 'Nội Lực', 'Thân Pháp', 'Thể Lực'] as const;
  const STAT_VALUES = [
